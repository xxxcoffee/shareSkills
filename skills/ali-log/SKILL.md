---
name: ali-log
description: 阿里云日志服务(SLS)查询工具，支持拉取原始日志、查询分析日志、获取游标等操作。包含查询构建器、SQL模板、函数参考。所有配置从环境变量读取。
---
...
2. **注意时间范围** - 查询时间范围不能超过24小时（部分接口限制）
3. **处理分页** - 大量日志需要分页获取，使用offset参数
4. **异常处理** - 所有方法可能抛出LogException，需要适当处理
5. **游标有效期** - 获取的游标有有效期，过期后需要重新获取
6. **GROUP BY优化** - 避免对字符串列进行分组，使用数值计算代替
7. **使用近似函数** - 大数据量时使用approx_distinct代替count(distinct)
8. **避免字符串生成** - 时间分组时使用date_trunc，避免date_format
